DELIMITER %
DROP PROCEDURE IF EXISTS `luna_dev_db`.`sp_chat_set_follower_delivered_sequence`%
CREATE PROCEDURE `luna_dev_db`.`sp_chat_set_follower_delivered_sequence`(
	  IN in_cid BIGINT  
...
-- EXAMPLE:
-- CALL `luna_dev_db`.`sp_chat_set_follower_delivered_sequence`(6, 1);
