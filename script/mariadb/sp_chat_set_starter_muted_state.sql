DELIMITER %
DROP PROCEDURE IF EXISTS `luna_dev_db`.`sp_chat_set_starter_muted_state`%
CREATE PROCEDURE `luna_dev_db`.`sp_chat_set_starter_muted_state`(
	  IN in_cid BIGINT
	, IN in_is_muted BOOL
)
BEGIN
	...
	SELECT cid, starter_id, starter_is_muted, last_message_sequence 
	INTO v_cid, v_starter_id, v_starter_is_muted, v_last_message_sequence 
	...
	IF(ISNULL(v_cid)) THEN
		SELECT 2 AS 'RC';
	ELSEIF(v_starter_is_muted = in_is_muted) THEN
		SELECT 3 AS 'RC';
	ELSE
...
-- EXAMPLE:
-- CALL `luna_dev_db`.`sp_chat_set_starter_muted_state`(6, FALSE);
