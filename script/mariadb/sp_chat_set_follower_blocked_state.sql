...
BEGIN
	...
	SELECT cid, follower_id, follower_is_blocked, last_message_sequence 
	INTO v_cid, v_follower_id, v_follower_is_blocked, v_last_message_sequence 
	...
	IF(ISNULL(v_cid)) THEN
		SELECT 2 AS 'RC';
	ELSEIF(v_follower_is_blocked = in_is_blocked) THEN
		SELECT 3 AS 'RC';
	ELSE
		...
		UPDATE luna_dev_db.chat_meta 
			SET follower_is_blocked = in_is_blocked
...
-- EXAMPLE:
-- CALL `luna_dev_db`.`sp_chat_set_follower_blocked_state`(6, FALSE);
