DELIMITER %
DROP PROCEDURE IF EXISTS `luna_dev_db`.`sp_chat_set_starter_seen_sequence`%
CREATE PROCEDURE `luna_dev_db`.`sp_chat_set_starter_seen_sequence`(
	  IN cid_ BIGINT  
...
-- EXAMPLE:
-- CALL `luna_dev_db`.`sp_chat_set_starter_seen_sequence`(6, 1);
