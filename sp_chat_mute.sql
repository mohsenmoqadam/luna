DELIMITER %
DROP PROCEDURE IF EXISTS `luna_dev_db`.`sp_chat_mute`%
CREATE PROCEDURE `luna_dev_db`.`sp_chat_mute`(
	  IN cid_ BIGINT
	, IN uid_ BIGINT
)
BEGIN
	...
	-- 1: EXCEPTION
	-- 2: INVALID CID
	-- 3: STARTER HAS ALREADY BEEN MUTED
	-- 4: FOLLOWER HAS ALREADY BEEN MUTED
	-- 5: FOLLOWER MUTED THE STARTER
	-- 6: STARTER MUTED THE FOLLOWER
	...
	END;
   	...
   	SET @cra = @mda;
   	SET @cid = NULL;
	SET @starter_is_muted = TRUE;
	SET @follower_is_muted = TRUE;
	SET @starter_id = NULL;
	...
	ELSE
		-- TRANSACTION: START
		START TRANSACTION;
		-- Starter has already been muted.
		IF (@starter_id = uid_ AND @starter_is_muted = TRUE) THEN
			SET @rc = 3;	
		-- Follower has already been muted.
		ELSEIF (@follower_id = uid_ AND @follower_is_muted = TRUE) THEN
			SET @rc = 4;	
		-- One side of the chat wants to block other side.
		ELSE
			-- Follower wants to mute the starter.
			IF (@starter_id = uid_ ) THEN
				...
				SET mda = @mda
				WHERE cid = @cid;
				-- Insert MUTE action message
				INSERT INTO `luna_dev_db`.`chat_message` (cid, cra, type, sequence, writer, body)
				VALUES (@cid, @cra, 'MUTE', @new_last_message_sequence, @follower_id, "");
			    -- Upddate RC
				SET @rc = 5;
			-- Starter wants to mute the follower.
			ELSEIF(@follower_id = uid_) THEN
				...
				-- Insert MUTE action message
				INSERT INTO `luna_dev_db`.`chat_message` (cid, cra, type, sequence, writer, body)
				VALUES (@cid, @cra, 'MUTE', @new_last_message_sequence, @starter_id, "");
			    -- Upddate RC
				SET @rc = 6;
			END IF;
		END IF;	
...
-- EXAMPLE:
-- CALL `luna_dev_db`.`sp_chat_mute`(5, 1);
